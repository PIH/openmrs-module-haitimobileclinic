package org.openmrs.module.haitimobileclinic.service;
...
import org.openmrs.module.haitimobileclinic.HaitiMobileClinicSearch;
...
import org.openmrs.module.haitimobileclinic.service.db.HaitiMobileClinicDAO;
import org.openmrs.module.haitimobileclinic.util.DuplicatePatient;
import org.openmrs.util.OpenmrsConstants;
import org.openmrs.util.OpenmrsUtil;
import org.springframework.transaction.annotation.Transactional;

public class HaitiMobileClinicServiceImpl implements HaitiMobileClinicService {

	protected final Log log = LogFactory.getLog(getClass());

	//***** PROPERTIES *****
	private HaitiMobileClinicDAO dao;
	
	//***** GETTERS AND SETTERS ****
	
	public HaitiMobileClinicDAO getDao() {
		return dao;
	}

	public void setDao(HaitiMobileClinicDAO dao) {
		this.dao = dao;
	}
	...
	public List<Patient> exactSearch(PersonName personName) {
		...
		return getHaitiMobileClinicSearch().exactSearch(personName);
    }
	...
	public List<Patient> search(PersonName personName) {
		...
		return getHaitiMobileClinicSearch().search(personName);
    }
	...
	public List<Patient> search(String personFirstName) {
		...
		return getHaitiMobileClinicSearch().search(personFirstName);
    }
	@Transactional(readOnly=true)
	...
	public List<Patient> search(Patient patient) {
		...
		return getHaitiMobileClinicSearch().search(patient);
    }
	...
	public boolean printRegistrationLabel(Patient patient, Location location, Integer count) {
		...
		try {
			...
			String ipAddress = HaitiMobileClinicGlobalProperties.GLOBAL_PROPERTY_LABEL_PRINTER_IP_ADDRESS();
			
			if (ipAddress == null) {
				throw new APIException("No ip address specified for label printer in global property");
			}
			
			Integer port = HaitiMobileClinicGlobalProperties.GLOBAL_PROPERTY_LABEL_PRINTER_PORT();
					...
					while(iter.hasNext()){
						...
						for (Map<String,String> token : line) {
							...
							if(token.get("isToken").equals(AddressSupport.getInstance().getDefaultLayoutTemplate().getLayoutToken())) {
								...
								String property = HaitiMobileClinicUtil.getPersonAddressProperty(patient.getPersonAddress(), token.get("codeName"));
			...
			data.append("^FO140,400^ATN^FD" + Context.getMessageSourceService().getMessage("haitimobileclinic.gender." + patient.getGender()) + "^FS"); 	
			
	
			/* RIGHT COLUMN */
			
			/* Print the numero dossier for the current location */
			PatientIdentifier numeroDossier = HaitiMobileClinicUtil.getNumeroDossier(patient, location);
			
			if (numeroDossier != null) {
				data.append("^FO870,50^FB350,1,0,R,0^AVN^FD" + numeroDossier.getIdentifier() + "^FS"); 
				data.append("^FO870,125^FB350,1,0,R,0^ATN^FD" + location.getName() + " " + Context.getMessageSourceService().getMessage("haitimobileclinic.dossier") + "^FS"); 
				data.append("^FO870,175^FB350,1,0,R,0^ATN^FD" + Context.getMessageSourceService().getMessage("haitimobileclinic.issued") + " " + df.format(new Date()) + "^FS"); 
			}
			
			/* Print the bar code, based on the primary identifier */
			PatientIdentifier primaryIdentifier = HaitiMobileClinicUtil.getPrimaryIdentifier(patient);
	...
	public boolean printIDCardLabel(Patient patient, Location location) {
		...
		try {
			...
			//List<PatientIdentifier> patientIdentifiers = HaitiMobileClinicUtil.getAllNumeroDossiers(patient);
			...
			if(identifierType!=null){				
				dossierIdentifier = HaitiMobileClinicUtil.getNumeroDossier(patient, location);
			}
			...
			if (dossierIdentifier != null) {												
					...
					data.append("^FO" + horizontalPosition + "," + (verticalPosition + 75) + "^ATN^FD" + dossierIdentifier.getLocation().getName() + " " + Context.getMessageSourceService().getMessage("haitimobileclinic.dossier") + "^FS"); 
			...
			if(identifierType!=null){				
				dossierIdentifier = HaitiMobileClinicUtil.getDentalDossier(patient, location);
			}
		
			/* Print all number dossiers in two columns*/
			if (dossierIdentifier != null ) {															
					data.append("^FO" + horizontalPosition + "," + verticalPosition + "^AVN^FD" + dossierIdentifier + "^FS"); 
					data.append("^FO" + horizontalPosition + "," + (verticalPosition + 75) + "^ATN^FD" + dossierIdentifier.getLocation().getName() + " " + Context.getMessageSourceService().getMessage("haitimobileclinic.menu.dentalDossier") + "^FS"); 
	...
	public boolean printDentalDossierLabel(Patient patient, Location location, int copies) {
		...
		try {
			...
			PatientIdentifierType identifierType = HaitiMobileClinicGlobalProperties.GLOBAL_PROPERTY_DENTAL_DOSSIER();
			List<PatientIdentifier> patientIdentifiers = HaitiMobileClinicUtil.getAllNumeroDossiers(patient, identifierType);
			...
			if (patientIdentifiers != null && patientIdentifiers.size() > 0) {	
				...
				for (PatientIdentifier identifier : patientIdentifiers) {
					...
					data.append("^FO" + horizontalPosition + "," + (verticalPosition + 75) + "^ATN^FD" + identifier.getLocation().getName() + " " + Context.getMessageSourceService().getMessage("haitimobileclinic.menu.dentalDossier") + "^FS"); 
	...
	public boolean printIDCard(Patient patient, Location location) {
		...
		try {
			...
			// make sure we have a ip address and port specified
			String ipAddress = HaitiMobileClinicGlobalProperties.GLOBAL_PROPERTY_ID_CARD_PRINTER_IP_ADDRESS();
			
			if (ipAddress == null) {
				throw new APIException("No ip address specified for id card printer in global property");
			}
			
			Integer port = HaitiMobileClinicGlobalProperties.GLOBAL_PROPERTY_ID_CARD_PRINTER_PORT();
			...
			data.append(ESC + "B 75 550 0 0 0 3 100 0 "+ HaitiMobileClinicUtil.getPreferredIdentifier(patient) + "\n");    // layout bar code and patient identifier
			data.append(ESC + "T 75 600 0 1 0 45 1 "+ HaitiMobileClinicUtil.getPreferredIdentifier(patient) + "\n");   
			...
			data.append(ESC + "T 75 350 0 0 0 25 1 " + Context.getMessageSourceService().getMessage("haitimobileclinic.gender") + "\n");
			data.append(ESC + "T 75 400 0 1 0 50 1 " + Context.getMessageSourceService().getMessage("haitimobileclinic.gender." + patient.getGender()) + "\n");
			
			data.append(ESC + "T 250 350 0 0 0 25 1 " + Context.getMessageSourceService().getMessage("haitimobileclinic.person.birthdate") + 
						(patient.getBirthdateEstimated() ? " (" + Context.getMessageSourceService().getMessage("haitimobileclinic.person.birthdate.estimated") + ")" : " ") + "\n");
			...
			PersonAttributeType type = HaitiMobileClinicGlobalProperties.GLOBAL_PROPERTY_ID_CARD_PERSON_ATTRIBUTE_TYPE();
			if (type != null) {
				...
				if (attr != null && attr.getValue() != null) {
					...
					String typeName = Context.getMessageSourceService().getMessage("haitimobileclinic." + HaitiMobileClinicUtil.toCamelCase(type.getName()), null, type.getName(), Context.getLocale());
			...
			if (HaitiMobileClinicGlobalProperties.GLOBAL_PROPERTY_ID_CARD_LABEL_TEXT() != null) {
				data.append(ESC + "T 420 510 0 1 0 45 1 " + HaitiMobileClinicGlobalProperties.GLOBAL_PROPERTY_ID_CARD_LABEL_TEXT() + "\n");
			}
			
			// date issued and location issued are aligned to bottom of the card
			data.append(ESC + "T 420 550 0 0 0 25 1 " + Context.getMessageSourceService().getMessage("haitimobileclinic.dateIDIssued") + "\n");
			data.append(ESC + "T 420 600 0 1 0 50 1 " + df.format(new Date()) + "\n");    // date issued is today
	
			data.append(ESC + "T 720 550 0 0 0 25 1 " + Context.getMessageSourceService().getMessage("haitimobileclinic.locationIssued") + "\n");
    ...
    /**
     * @see HaitiMobileClinicService#getNumberOfRegistrationEncounters(List<EncounterType>, Date, Date)
     */
    ...
    /**
     * @see HaitiMobileClinicService#getNumberOfEncountersByDate(EncounterType, Location)
     */
	// TODO: do we want to move this to the DAO?
	@Transactional(readOnly=true)
    ...
    /**
     * @see HaitiMobileClinicService#getNumberOfPatientsByAddress(Map<String, String>, String, EncounterType, Location)
     */
	@Transactional(readOnly=true)
    ...
    /**
     * @see HaitiMobileClinicService#getNumberOfEncountersByAddress(Map<String, String>, String, EncounterType, Location)
     */
	@Transactional(readOnly=true)
	...
	private Encounter getEncounterByDateAndType(Patient patient, EncounterType encounterType, Location location, Date registrationDate) {
		...
		Date startTime = HaitiMobileClinicUtil.clearTimeComponent(registrationDate);
	 ...
	 */
    private HaitiMobileClinicSearch getHaitiMobileClinicSearch() {
		String patientSearchClass = HaitiMobileClinicGlobalProperties.GLOBAL_PROPERTY_SEARCH_CLASS();
		
		// if the global prop doesn't exist, use the hard-coded default class
		if (StringUtils.isBlank(patientSearchClass)) {
			patientSearchClass = HaitiMobileClinicConstants.DEFAULT_SEARCH_CLASS;
		}
        ...
        try {
        	return (HaitiMobileClinicSearch) patientSearch.newInstance();
        }
        catch(Exception e) {
    ...
    /**
     * @see HaitiMobileClinicService#saveUserActivity(UserActivity)
     */
    @Transactional
