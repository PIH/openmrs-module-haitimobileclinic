...
import org.openmrs.module.haitimobileclinic.util.PatientDiagnosis;
 ...
 */
@Controller
@RequestMapping(value = "/module/haitimobileclinic/workflow/primaryCareVisitEncounter.form")
public class PrimaryCareVisitEncounterController extends AbstractPatientDetailsController {
	...
	public ModelAndView showSelectPatient(
		...
		if (patient == null) {
			return new ModelAndView("redirect:/module/haitimobileclinic/workflow/primaryCareVisitTask.form");
		}
		PersonAttributeType providerIdentifierType=HaitiMobileClinicGlobalProperties.GLOBAL_PROPERTY_PROVIDER_IDENTIFIER_ATTRIBUTE_TYPE();
		if(providerIdentifierType!=null){
			model.addAttribute("providerIdentifierType", providerIdentifierType.getName());
		}
		
		List<User> providers = HaitiMobileClinicUtil.getProviders();
		...
		if(notifyConcept==null){
			...
			Map<String, String> notifyConceptMap = HaitiMobileClinicUtil.getConvSetMap(notifyConcept, icd10);
			if(notifyConceptMap!=null){
				model.addAttribute("notifyingDiseases", notifyConceptMap);
			}
		}
		Concept urgentConcept = HaitiMobileClinicGlobalProperties.GLOBAL_PROPERTY_PRIMARY_CARE_VISIT_URGENT_DIAGNOSIS_CONCEPT();
		if(urgentConcept==null){
			log.error("Global property haitimobileclinic.primaryCareVisitUrgentDiagnosisConcept is undefined or does not match an existing concept");
		}else{
			Map<String, String> urgentConceptMap = HaitiMobileClinicUtil.getConvSetMap(urgentConcept, icd10);
			if(urgentConceptMap!=null){
				model.addAttribute("urgentDiseases", urgentConceptMap);
			}
		}
		
		Concept ageRestrictedConcept = HaitiMobileClinicGlobalProperties.GLOBAL_PROPERTY_PRIMARY_CARE_VISIT_AGE_RESTRICTED_CONCEPT();
		if(ageRestrictedConcept==null){
			log.error("Global property haitimobileclinic.primaryCareVisitAgeRestrictedConcept is undefined or does not match an existing concept");
		}else{
			Map<String, String> ageRestrictedConceptMap = HaitiMobileClinicUtil.getConvSetMap(ageRestrictedConcept, icd10);
			if(ageRestrictedConceptMap!=null){
				model.addAttribute("ageRestrictedDiseases", ageRestrictedConceptMap);
			}
		}
		
		Concept nonCodedConcept=HaitiMobileClinicGlobalProperties.GLOBAL_PROPERTY_PRIMARY_CARE_VISIT_NON_CODED_DIAGNOSIS_CONCEPT();
		if(nonCodedConcept!=null){
			List<String> nonCoded = Context.getService(HaitiMobileClinicService.class).getDistinctObs(nonCodedConcept.getId());
			...
			if(editEncounterId!=null){
				try{
					...
					if(editEncounter!=null){
						...
						try{
							...
							if(encounterProviderUsers!=null && encounterProviderUsers.size()>0){
								for(User user : encounterProviderUsers){
									if(HaitiMobileClinicUtil.doesUserHaveProviderRole(user)){
						...
						obs = HaitiMobileClinicWebUtil.getPatientDiagnosis(patient, encounterType, editEncounter, registrationLocation, encounterDate);
					}
				}catch(Exception e){
		...
		EncounterType receptionEncounterType = HaitiMobileClinicGlobalProperties.GLOBAL_PROPERTY_PRIMARY_CARE_RECEPTION_ENCOUNTER_TYPE();
		...
		if(encounters!=null && encounters.size()>0){
			...
			for(Encounter receptionEncounter : encounters){				
				...
				List<Obs> diagnosisObs = HaitiMobileClinicWebUtil.getPatientDiagnosis(patient, encounterType, null, registrationLocation, receptionEncounter.getEncounterDatetime());					
				List<POCObservation> patientObservation=HaitiMobileClinicWebUtil.getPOCObservation(diagnosisObs, icd10);
		...
		return new ModelAndView("/module/haitimobileclinic/workflow/primaryCareVisitEncounter");	
	}
	...
	public ModelAndView processDiagnosis(
		...
		if(StringUtils.isNotBlank(diagnosisList)){
			List<Obs> observations = HaitiMobileClinicUtil.parseDiagnosisList(diagnosisList);
			...
			if(observations!=null && observations.size()>0){				
				...
				List<Obs> currentObs = HaitiMobileClinicWebUtil.getPatientDiagnosis(patient, encounterType, null, registrationLocation, encounterDate.getTime());
				...
				if(taskProgress!=null){
					...
					taskProgress.setProgressBarImage(HaitiMobileClinicConstants.RETROSPECTIVE_PROGRESS_4_IMG);			
				...
				encounter.setEncounterDatetime(HaitiMobileClinicUtil.clearTimeComponent(encounterDate.getTime()));
				encounter.setEncounterType(encounterType);
				encounter.setProvider(Context.getAuthenticatedUser().getPerson());
				encounter.setLocation(HaitiMobileClinicWebUtil.getRegistrationLocation(session));
