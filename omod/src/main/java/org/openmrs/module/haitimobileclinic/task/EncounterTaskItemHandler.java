package org.openmrs.module.haitimobileclinic.task;
...
public class EncounterTaskItemHandler {
	...
	public ModelAndView render(EncounterTaskItem taskItem, ModelMap model) {
		...
		return new ModelAndView("/module/HaitiMobileClinic/task/encounterTaskItem", model);
	}
	...
	public ModelAndView handleSubmit(EncounterTaskItem taskItem, Patient patient, HttpServletRequest request, ModelMap model) {
		...
		UserActivityLogger.logActivity(request.getSession(), HaitiMobileClinicConstants.ACTIVITY_ENCOUNTER_SAVED, "Encounter: " + e.getUuid());
