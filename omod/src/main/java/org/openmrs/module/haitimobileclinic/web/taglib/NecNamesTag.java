...
public class NecNamesTag extends TagSupport {
		...
		try {
			if (!"".equals(getReferralEncounterId())) {
				...
				Encounter e = Context.getEncounterService().getEncounter(
						encounterId);
				Iterator<Obs> obses = getObsesFromEncounter(e, Arrays.asList(Context.getConceptService().getConcept(6769), Context.getConceptService().getConcept(6795))).iterator();
				while (obses.hasNext()) {
