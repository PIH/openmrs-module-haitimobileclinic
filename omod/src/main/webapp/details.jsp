...
<script type="text/javascript">
	...
	<c:forEach var="scriptIndex" items="${jscriptMessages}">'${scriptIndex}',
		scriptMessages['${scriptIndex}']='<spring:message code="haitimobileclinic.jMessages.${scriptIndex}" javaScriptEscape="true"/>';
	</c:forEach>
	
	var toolTipMessages=[];	
	<c:forEach var="scriptIndex" items="${jtoolTipMessages}">'${scriptIndex}',
		toolTipMessages['${scriptIndex}']='<spring:message code="haitimobileclinic.toolTip.${scriptIndex}" javaScriptEscape="true"/>';
	</c:forEach>
	
	var addressLabels=[];
	<c:forEach var="addressLevel" items="${addressHierarchyLevels}">'${addressLevel}',
		addressLabels['${addressLevel}']='<spring:message code="haitimobileclinic.person.address.${addressLevel}" javaScriptEscape="true"/>';
	</c:forEach>
