...
<openmrs:htmlInclude file="/moduleResources/haitimobileclinic/jquery.flot.pie.min.js"/>
	...
	<table width="100%"><tr>
		<td valign="top" align="left" style="white-space:nowrap;">
			<form method="post" id="catchmentForm">
				...
				<br/><br/>
				<b><spring:message code="haitimobileclinic.encounterType"/></b><br/>
				<select name="encounterType" onchange="submitForm();">
					<option value=""${empty encounterType ? ' selected' : ''}><spring:message code="haitimobileclinic.allEncounterTypes"/></option>
				...
				<br/><br/>
				<b><spring:message code="haitimobileclinic.catchmentArea"/></b><br/>
				<table>
					<c:choose>
						<c:when test="${empty filterCriteria}"><tr><td><spring:message code="haitimobileclinic.unspecified"/></td></tr></c:when>
						<c:otherwise>
					...
					<tr>
						<td valign="top" style="white-space:nowrap;">
							<table class="dataTable">
								<tr><th></th><th colspan="4" align="center"><spring:message code="haitimobileclinic.numberOfRegistrations"/></th></tr>
								<tr class="bottomHeaderRow">
									<th align="left">${nameMappings[nextField]}</th>
									<th><spring:message code="haitimobileclinic.distinctPatients"/></th>
									<th><spring:message code="haitimobileclinic.distinctEncounters"/></th>
								</tr>
								...
								<c:forEach items="other,unspecified" var="nextVal">
									...
									<c:set value="${numEncountersByAddress[nextVal]}" var="encCount"/>
									<tr>
										<td align="left"><spring:message code="haitimobileclinic.${nextVal}"/></td>
										<td>${patCount}</td>
										<td>${encCount}</td>
									</tr>
								</c:forEach>
						...
						<td valign="top" align="left">
							<table align="center" id="monthYearTable" style="padding-bottom:20px;">
								<tr>
									<th align="left"><spring:message code="haitimobileclinic.registrationBreakdownBy"/>:</th>
									<td><button id="encounterBreakdownButton" onclick="showEncounterBreakdown();"><spring:message code="haitimobileclinic.encounter"/></button></td>
									<td><button id="patientBreakdownButton" onclick="showPatientBreakdown();"><spring:message code="haitimobileclinic.patient"/></button></td>
								</tr>
							</table>
		...
		$j.plot($j("#chartFrame"), data, {
			...
			legend: {
				...
				labelFormatter: function(label, series) {
					var l = label;
					if (l == 'other') {
						l = '<spring:message code="haitimobileclinic.other"/>';
					}
					if (l == 'unspecified') {
						l = '<spring:message code="haitimobileclinic.unspecified"/>';
					}
