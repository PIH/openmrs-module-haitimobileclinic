...
<div id="content" style="padding-top:20px;"/>
	<table class="locationTable">
		<tr>
			<td class="${empty location ? 'selected' : ''}" onclick="document.location.href='overviewReport.form'">
				<spring:message code="haitimobileclinic.allLocations"/>
			</td>
			...
			<td valign="top" style="white-space:nowrap;">
				<table class="dataTable">
					...
					<tr class="bottomHeaderRow">
						<th></th>
						<th><spring:message code="haitimobileclinic.today"/></th>
						<th><spring:message code="haitimobileclinic.pastWeek"/></th>
						<th><spring:message code="haitimobileclinic.pastMonth"/></th>
						<th><spring:message code="haitimobileclinic.ever"/></th>
						<th></th>
					</tr>
						...
						<tr>
							...
							<td>
								<button onclick="renderChart('${type.encounterTypeId}', '${type.name}')">
									<spring:message code="haitimobileclinic.viewDetails"/>
								</button>
							</td>
						</tr>
					</c:forEach>
				...
				<table width="600px;" id="monthYearTable" style="display:none; padding-top:20px;">
					...
					<tr>
						<c:forEach begin="0" end="11" var="month">
							<td><button id="monthButton${month}" class="monthButton" onclick="changePeriod(this, 'month', ${month})"><spring:message code="haitimobileclinic.month.${month+1}"/></button></td>
						</c:forEach>
					</tr>
				</table>
			</td>
	...
	<script type="text/javascript">
		...
		var monthNames = [<c:forEach begin="1" end="12" var="m" varStatus="mStatus">'<spring:message code="haitimobileclinic.month.${m}"/>'<c:if test="${!mStatus.last}">,</c:if></c:forEach>];
		var chartTitle;
		var chartData;
		var today = new Date();
		var currYear;
		...
		function renderChart(encounterTypeId, encounterTypeName) {
			chartTitle = '<spring:message code="haitimobileclinic.numRegistrationsByType" arguments="' + encounterTypeName + '"/>';
			<c:if test="${!empty location}">chartTitle='<spring:message code="haitimobileclinic.numRegistrationsByTypeAtLocation" arguments="' + encounterTypeName + ', ${location.name}"/>';</c:if>
