$j(document).ready(function(){	
	...
	if(cardPrintedStatus == 'true'){			
		$j("#printedIdCard").attr('src', pageContextAddress + '/moduleResources/haitimobileclinic/images/card-printed-icon.png');
	}else{
		$j("#printedIdCard").attr('src', pageContextAddress + '/moduleResources/haitimobileclinic/images/no-card-icon.png');
	}
	...
	*/
	...
	$j.removeDuplicatePatient =  function(mapId, patientListId){
		...
		$j.ajax({
			type: 'POST',
			async: false,
			url: pageContextAddress + '/module/haitimobileclinic/ajax/removeFalseDuplicates.form',
			//dataType: 'json',
	...
	function addDuplicatePatient(mapId, patientListId){
		...
		$j.ajax({
			type: 'POST',
			async: false,
			url: pageContextAddress + '/module/haitimobileclinic/ajax/addPatientDuplicate.form',			
			data: { 'patientId': mapId, 'duplicateId': patientListId },
			success: function(elements) {													
		...
		if ($j('#confirmExistingPatientModalDiv').is(':visible') ){
			if (event.keyCode == KEYCODE_ARROW_UP){				
			  ...
			  if((selectedMatchItem!==null)){		
				...
				window.location.href=pageContextAddress + '/module/haitimobileclinic/workflow/patientDashboard.form?patientId=' + selectedRowId ;
	...
	$j( "#confirmExistingPatientModalDiv" ).dialog({
			...
			open: function(event, ui){		
				...
				$j("#cancelBtn").css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/cross-red.png')");
			},
			...
			*/
			...
			$j.each(duplicatePatientsData, function(i,duplicatePatient) {								
				...
				columnModal.click(function(){																				
						alertUserAboutLeaving = false;
						window.location.href=pageContextAddress + '/module/haitimobileclinic/workflow/patientDashboard.form?patientId=' + duplicatePatient.patientId;
						setTimeout('$j("#confirmExistingPatientModalDiv").dialog("close"); ', 1000);			
				});
				...
				.click(function(event){
					...
					$j("#dialog-confirmDuplicate").dialog({
						...
						open: function(event, ui){								
							...
							$j("#doNotAddBtn").css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/z-red.png') center center no-repeat");	
							
							$j("#confirmDuplicateBtn").addClass('modalConfirmBtn');
							$j("#confirmDuplicateBtn").css("border", "0");
							$j("#confirmDuplicateBtn").css("float", "right");
							$j("#confirmDuplicateBtn").css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/checkmark-yellow.png') center center no-repeat");
				...
				addDuplicateBtn.css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/thumbsUp.jpg') center center no-repeat");
				...
				.click(function(event){
					...
					$j("#dialog-removeDuplicate").dialog({
						...
						open: function(event, ui){								
							...
							$j("#doNotRemoveBtn").css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/z-red.png') center center no-repeat");	
							
							$j("#removeDuplicateBtn").addClass('modalConfirmBtn');
							$j("#removeDuplicateBtn").css("border", "0");
							$j("#removeDuplicateBtn").css("float", "right");
							$j("#removeDuplicateBtn").css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/checkmark-yellow.png') center center no-repeat");
				...
				removeDuplicateBtn.css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/thumbsDown.jpg') center center no-repeat");
...
//				id: "yesBtn",
//				click: function() {						
//					$j.goToNextPage(nextTask, '/module/haitimobileclinic/workflow/patientDashboard.form?createEncounter=haitiMobileClinicEncounterType&cardPrinted=true&patientId=' + patientId );					
...
//				id: "noBtn",
//				click: function() {						
//					$j.goToNextPage(nextTask, '/module/haitimobileclinic/workflow/patientDashboard.form?createEncounter=haitiMobileClinicEncounterType&cardPrinted=false&patientId=' + patientId );					
//					setTimeout('$j("#dialog-confirm").dialog("close"); ', 1000);
//				}
//			}				
//		]		
...
//			$j("#noBtn").css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/z-red.png') center center no-repeat");				
//			$j("#yesBtn").css(cssObj);
//			$j("#yesBtn").css("float", "right");
//			$j("#yesBtn").css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/checkmark-yellow.png')");
//			$j("#yesBtn").focus();							
//			
//		}
//	});
	...
	$j('#checkmark-yellow').click(function(event) {
		if(nextTask.length>0){
			window.location.href=pageContextAddress + '/module/haitimobileclinic/workflow/' + nextTask + '?patientId=' + patientId;
		}else{
			window.location.href=pageContextAddress + '/module/haitimobileclinic/workflow/' + taskName + 'Task.form';
		}
	});
	...
	$j(".editDemoDiv").click(function(event){
		var divId = this.id;		
		$j.goToNextPage(nextTask, '/module/haitimobileclinic/workflow/enterPatientDemo.form?patientId='+patientId + "&editDivId=" + divId);		
	});
	
//	$j("#brokenPrinterBtn").click(function(event) {
//		$j.goToNextPage(nextTask, '/module/haitimobileclinic/workflow/patientDashboard.form?cardPrinted=false&patientId=' + patientId);
//	});
...
//			}else{
//				window.location.href=pageContextAddress + '/module/haitimobileclinic/workflow/patientDashboard.form?createEncounter=haitiMobileClinicEncounterType&cardPrinted=true&patientId=' + patientId ;				
//			}
//		  }else{
