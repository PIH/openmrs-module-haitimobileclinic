$j(document).ready(function(){				
	...
	$j( "#confirmPossibleLocalityModalDiv" ).dialog({
			...
			open: function(event, ui){		
				...
				$j("#cancelFirstBtn").css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/cross-red.png')");
		...
		if (event.keyCode == 13 ) {
		  ...
		  if( scanedPatientIdentifier.length>1){
			...
			if(scanedPatientIdentifier !== editPatientIdentifier){				
				...
				$j('#confirmPatientModalDiv').load(pageContextAddress + '/module/haitimobileclinic/workflow/confirmPatient.form?patientIdentifier=' + scanedPatientIdentifier + ' #modalTable', 
					function(){				
						$j.hideOkButton();	
						$j('#scanPatientIdentifier').val("");
				});
...
//			$j.setScanIdCardDiv();
	...
	*/
	...
	$j('#cross-red').click(function(event){
		window.location.href=pageContextAddress + '/module/haitimobileclinic/workflow/patientRegistrationTask.form';
	});
		...
		if ($j('#confirmExistingPatientModalDiv').is(':visible') ){
			if (event.keyCode ==38){
			  ...
			  if((selectedMatchItem!==null)){		
				...
				$j.goToNextPage(nextTask, '/module/haitimobileclinic/workflow/patientDashboard.form?createId=true&createEncounter=haitiMobileClinicEncounterType&patientId=' + selectedRowId );
			...
			*/
			...
			$j.each(patientArray, function(i,patient) {
				...
				if(patientId.length>0 && patient.id==patientId){
					...
					rowModal.click(function(){																				
							alertUserAboutLeaving = false;
							$j.goToNextPage(nextTask, '/module/haitimobileclinic/workflow/patientDashboard.form?createId=true&createEncounter=haitiMobileClinicEncounterType&patientId=' + patient.id );										
							setTimeout('$j("#confirmExistingPatientModalDiv").dialog("close"); ', 1000);			
					});
			...
			$j('#confirmExistingPatientDiv').click(function(){											
				if (patientsLength>1){
					...
					$j("#confirmPatientModalDiv").dialog("open");
					$j('#confirmPatientModalDiv').load(pageContextAddress + '/module/haitimobileclinic/workflow/confirmPatient.form?patientId=' + patientIdentifier + ' #modalTable');
				}				
			});					
			$j('#messageArea').show();		
			$j('#matchedPatientDiv').css("visibility", "visible");
	...
	$j.searchExistingPatients = function() {
		...
		jqxhr = $j.getJSON(pageContextAddress + '/module/haitimobileclinic/ajax/patientSearch.form'
			...
			if(patientsLength>0){
				...
				if(patientsLength<21){
					var loaderImg =$j(document.createElement('img')).attr(
							{src: pageContextAddress  + "/moduleResources/haitimobileclinic/images/smallwhiteloader.gif", 
							 id: 'smallerLoaderId'
							});
	...
	$j( "#confirmPatientModalDiv" ).dialog({
		...
		open: function(event, ui){		
			...
			$j("#cancelBtnConf").css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/left-arrow-white.png') center center no-repeat");	
			
			$j("#okBtnConf").css(cssObj);
			$j("#okBtnConf").css("float", "right");
			$j("#okBtnConf").css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/checkmark-yellow.png') center center no-repeat");
		...
		buttons: [
			...
			},
			{
				...
				click: function() {						
					alertUserAboutLeaving = false;
					$j.goToNextPage(nextTask, '/module/haitimobileclinic/workflow/patientDashboard.form?createId=true&createEncounter=haitiMobileClinicEncounterType&patientId=' + patientIdentifier + "&patientIdentifier="+ scanedPatientIdentifier + "&cardPrinted=" + cardPrinted);					
	...
	$j.searchSoundexPatients = function(exactMatchNo) {
		...
		jqSoundXHR = $j.getJSON(pageContextAddress + '/module/haitimobileclinic/ajax/patientSoundexSearch.form'
					, $j('#patientSearch').serialize()
			...
			*/		
			...
			if(searchName.length>1){
				...
				patientNameTimerId= setTimeout(function() {  
					namexhr = $j.getJSON(pageContextAddress + '/module/haitimobileclinic/ajax/patientNameOccurrencesSearch.form'
	...
	$j("#brokenPrinterBtn").click(function(event) {
		$j.goToNextPage(nextTask, '/module/haitimobileclinic/workflow/patientDashboard.form?cardPrinted=false&createId=true&createEncounter=haitiMobileClinicEncounterType&patientId=' + patientId);
	});
	
	$j("#reprintIDCardBtn").click(function(event) {
		$j.goToNextPage(nextTask, '/module/haitimobileclinic/workflow/enterPatientDemo.form?hiddenPrintIdCard=yes&patientId=' + patientId);
	});
