$j(document).ready(function(){
	...
	$j.lookupPatients = function(searchString, resultsArray, ajaxAddress) {
		...
		patientNameTimerId= setTimeout(function() {  
			requestCounter++;
			jqxhr  = $j.getJSON(
				pageContextAddress + '/module/haitimobileclinic/ajax/' + ajaxAddress
	...
	$j('#right-arrow-yellow').click(function(event){				
		if(patientId.length>0){
			...
			window.location.href=pageContextAddress + '/module/haitimobileclinic/workflow/patientDashboard.form?patientId=' + patientId;
		}
	});
	...
	$j( "#confirmPatientModalDiv" ).dialog({
		...
		open: function(event, ui){		
			...
			$j("#cancelBtn").css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/left-arrow-white.png')");			
			$j("#okBtn").css(cssObj);
			$j("#okBtn").css("float", "right");			
			$j("#okBtn").css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/checkmark-yellow.png')");			
			$j("#okBtn").focus();								
		}, 
		buttons: [
			{
