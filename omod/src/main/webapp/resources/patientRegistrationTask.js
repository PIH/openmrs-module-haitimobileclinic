$j(document).ready(function(){
		...
		if ($j('#confirmExistingPatientModalDiv').is(':visible') ){
			if (event.keyCode ==38){
			  ...
			  if((selectedItem!==null)){								
				...
				var nextPage = '/module/haitimobileclinic/workflow/patientDashboard.form?createId=true&createEncounter=haitiMobileClinicEncounterType&patientId=' + selectedRowId ;
	...
	$j.searchByIdentifier = function(idValue) {		
		...
		patientsLength=0;		
		$j.getJSON(pageContextAddress + '/module/haitimobileclinic/ajax/patientIdentifierSearch.form'
						...
						if( patientsLength > 0){														
							$j.each(patients, function(i, patient){
								if(patientsLength ==1){
										...
										$j('#confirmPatientModalDiv').load(pageContextAddress + '/module/haitimobileclinic/workflow/confirmPatient.form?patientId=' + patientId + ' #modalTable');							
									     return;
								}
								...
								rowModal.click(function(){	
										var nextPage = '/module/haitimobileclinic/workflow/patientDashboard.form?createId=true&createEncounter=haitiMobileClinicEncounterType&patientId=' + patient.id ;
										if(nextTask.length>0){
											nextPage = nextPage + '&nextTask=' + nextTask;
										}
										window.location.href=pageContextAddress + nextPage;										
	...
	$j('#searchByNameBtn').click(function(event){
		...
		var nextPage = '/module/haitimobileclinic/workflow/enterPatientDemo.form';
		if(nextTask.length>0){
			nextPage = nextPage + '?nextTask=' + nextTask;
		}
		window.location.href=pageContextAddress + nextPage;	
	...
	$j( "#confirmPatientModalDiv" ).dialog({
		...
		open: function(event, ui){		
			...
			$j("#cancelBtn").css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/left-arrow-white.png') center center no-repeat");	
					
			$j("#okBtn").addClass('modalConfirmBtn');
			$j("#okBtn").css("float", "right");			
			$j("#okBtn").css("background", "url('" + pageContextAddress  + "/moduleResources/haitimobileclinic/images/checkmark-yellow.png') center center no-repeat");	
		...
		buttons: [
			...
			},
			{
				...
				id: "okBtn",
				click: function() {		
					var nextPage = '/module/haitimobileclinic/workflow/patientDashboard.form?createId=true&createEncounter=haitiMobileClinicEncounterType&patientId=' + patientId;
					if(nextTask.length>0){
						nextPage = nextPage + '&nextTask=' + nextTask;
					}
					window.location.href=pageContextAddress + 	nextPage;				
