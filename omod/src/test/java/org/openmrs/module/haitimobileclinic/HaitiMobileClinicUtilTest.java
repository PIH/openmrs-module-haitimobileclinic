...
public class HaitiMobileClinicUtilTest extends BaseModuleContextSensitiveTest {

	protected final Log log = LogFactory.getLog(getClass());

	protected static final String PATIENT_REGISTRATION_XML_DATASET_PACKAGE_PATH = "org/openmrs/module/HaitiMobileClinic/include/HaitiMobileClinic-dataset.xml";
	...
	public void getPatientIdentifierTypesToDisplay_shouldReturnAllPatientIdentifierTypes() throws Exception {
		...
		List<PatientIdentifierType> types = HaitiMobileClinicUtil.getPatientIdentifierTypesToDisplay();
	...
	public void getPatientIdentifierTypesToDisplay_shouldReturnPatientIdentifierTypesStoredInHaitiMobileClinicProp() throws Exception {
		
		// set both global property 
		Context.getAdministrationService().saveGlobalProperty(new GlobalProperty("patient_identifier.importantTypes", "OpenMRS Identification Number:Somewhere,HIV Program Number:Somewhere else"));
		Context.getAdministrationService().saveGlobalProperty(new GlobalProperty("HaitiMobileClinic.identifierTypes", "Old Identification Number|MDR-TB Program Number"));
	...
	public void getPatientIdentifierTypesToDisplay_shouldIncludePrimaryIdentifierType() throws Exception {
		...
		Context.getAdministrationService().saveGlobalProperty(new GlobalProperty("patient_identifier.importantTypes", "OpenMRS Identification Number:Somewhere,HIV Program Number:Somewhere else"));
		Context.getAdministrationService().saveGlobalProperty(new GlobalProperty("HaitiMobileClinic.identifierTypes", "Old Identification Number|MDR-TB Program Number"));
		Context.getAdministrationService().saveGlobalProperty(new GlobalProperty("HaitiMobileClinic.primaryIdentifierType", "HIV Program Number"));
	...
	public void calculateBirthdateFromAge_shouldCalculateBirthdateBasedOnAge() throws Exception {
		...
		Date birthdate = HaitiMobileClinicUtil.calculateBirthdateFromAge(0, 0, 2, ageOnDate);
		...
		birthdate = HaitiMobileClinicUtil.calculateBirthdateFromAge(0, 0, 51, ageOnDate);
		...
		birthdate = HaitiMobileClinicUtil.calculateBirthdateFromAge(0, 4, 0, ageOnDate);
		...
		birthdate = HaitiMobileClinicUtil.calculateBirthdateFromAge(0, 16, 0, ageOnDate);
		...
		birthdate = HaitiMobileClinicUtil.calculateBirthdateFromAge(0, 6, 4, ageOnDate);
		...
		birthdate = HaitiMobileClinicUtil.calculateBirthdateFromAge(38, 0, 0, ageOnDate);
		c.set(1972, Calendar.JANUARY, 1, 0, 0, 0);
		Assert.assertEquals(c.getTime(), birthdate);
		
		// try someone 25 years, 3 months, and 6 days old -- birth date should be December 28, 1984 
		birthdate = HaitiMobileClinicUtil.calculateBirthdateFromAge(25, 3, 6, ageOnDate);
		...
		birthdate = HaitiMobileClinicUtil.calculateBirthdateFromAge(12, 8, 0, ageOnDate);
		...
		birthdate = HaitiMobileClinicUtil.calculateBirthdateFromAge(9, 0, 35, ageOnDate);
		c.set(2001, Calendar.FEBRUARY, 27, 0, 0, 0);
		Assert.assertEquals(c.getTime(), birthdate);
		
		// make sure that the test cases I listed in the javadocs are correct
		birthdate = HaitiMobileClinicUtil.calculateBirthdateFromAge(10, 0, 0, ageOnDate);
		c.set(2000, Calendar.JANUARY, 1, 0, 0, 0);
		Assert.assertEquals(c.getTime(), birthdate);
		
		birthdate = HaitiMobileClinicUtil.calculateBirthdateFromAge(0, 2, 0, ageOnDate);
	...
	public void calculateAgeFromBirthdate_shouldCalculateAgeBasedOnBirthdate() throws Exception {
		...
		c.set(2010, Calendar.APRIL, 1, 0, 0, 0);
		Age age = HaitiMobileClinicUtil.calculateAgeFromBirthdate(c.getTime(), ageOnDate);
		...
		//c.set(2009, Calendar.SEPTEMBER, 29, 0, 0, 0);
		//age = HaitiMobileClinicUtil.calculateAgeFromBirthdate(c.getTime(), ageOnDate);
		//Assert.assertEquals(new Integer(0), age.getYears());
	...
	public void getPersonAddressProperty_shouldReturnPersonAddressProperty() throws Exception {
		...
		String cityVillage = HaitiMobileClinicUtil.getPersonAddressProperty(person.getPersonAddress(), "cityVillage");
		Assert.assertTrue(StringUtils.equals("Indianapolis", cityVillage));
		
		// try a field with no value
		String countyDistrict = HaitiMobileClinicUtil.getPersonAddressProperty(person.getPersonAddress(), "countyDistrict");
	...
	public void toCamelCase_shouldConvertStringToCamelCase() throws Exception {
		Assert.assertEquals("toCamelCase", HaitiMobileClinicUtil.toCamelCase("To camel Case"));
	}
}
